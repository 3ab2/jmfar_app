...
# Test Evenement File Handling API using curl
...
curl -s -X GET "$BASE_URL/evenements/$EVENEMENT_ID" | python -m json.tool
echo ""
...
curl -s -X GET "$BASE_URL/evenements/$EVENEMENT_ID" | python -c "import sys, json; data=json.load(sys.stdin); print(json.dumps(data['fichiers'], indent=2))"
echo ""

# Test 6: List all evenements
echo "6. Listing all evenements..."
curl -s -X GET "$BASE_URL/evenements" | python -c "import sys, json; data=json.load(sys.stdin); print(json.dumps([{'id': e['id'], 'titre': e['titre'], 'fichiers_count': len(e['fichiers'])} for e in data], indent=2))"
echo ""
